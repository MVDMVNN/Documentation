...
Готово, новый элемент отображается, “хвост” в пути к картинке изменился (обращаем на это внимание для самопроверки) - фото подгружено успешно ✅
...
Вводим в поисковик по коду сommand+F - “Hidden element”, для того чтоб быстрее добраться до фрагмента, который хранит в себе нужные нам фото
...
Обращаемся в конец пути нужного фото, в данном случае, productName и заменяем в style = “display: none” на style = “display: block”
...
В режиме Split начнет отображаться фото, которое подтягивается на оффер - prize.jpg, по такому же принципу заменяем его на нужное
...
После смены картинки снова обращаемся в конец пути и заменяем style = “display: block” на style = “display: none” и сохранить изменения
