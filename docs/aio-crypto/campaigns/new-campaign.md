...
С самого начала переходим в таблицу с доменами для AIO и резервируем уже закупленные или идём закупать новые и заносим в таблицу, если это `facebook` сетапы. Если это `google` сетапы, то необходимо купить их, проксировать и закинуть на сервер.
