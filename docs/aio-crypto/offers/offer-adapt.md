...
После того, как мы добавим новый код, далее, нужно удалить абсолютно все `.js` файлы в конце страницы, файл `send.php`. Из папки `thanks` мы переносим файл `index.html/index.php/thanks.html` в корень папки, а `.css` файл со стилями в `offer-assets` и прописываем новый путь в `thanks.html`.

Так же, в файле `thanks.html` нужно удалить все скрипты оптравки лидов и подключения `.js` файлов.

Мы ничего из этого более не используем, всё делается силами AIO и под его капотом.
